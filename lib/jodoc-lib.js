...
function html_header (body,title,template,links,index,name) {
	title = title || "MapQuery";
	links = links || "";
	index = index || "";
	var output = template || default_template;
	output = output.replace(/\$title/g,title);
	if(name == "_index.html") {links = ""}; 
	output = output.replace(/\$links/g,links);
...
// find and return all the h3 tags in the processed files
// input: [{name:"filename", content:"file content"}]
// output: {h3s: {h3:file_name,...}, files_to_h3s: {filename:[h3s_in_file],...}}
function h3finder (processed) {
    var h3s = {};
    var files_to_h3s = {};
    var h3find = /\<h3\>.*\<code\>([^\<]+)\<\/code\>\<\/h3>/g;
    var codefind = /\<code\>([^\<]+)\<\/code\>/g;
    processed.forEach(
        function(file_info) {
            var accum_h3s = [];
            var h3 = "";
            while ((h3 = h3find.exec(file_info.content)) !== null){
                while((h3 = codefind.exec(h3)) !==null){
                    accum_h3s.push(h3[1]);
                }
            }
            accum_h3s.forEach(
                function (h3) {
                    h3s[h3] = file_info.name;
                }
            );
            files_to_h3s[file_info.name] = accum_h3s;
        }
    );
    return {h3s:h3s, files_to_h3s:files_to_h3s};
}

function h3index (h3s, outputdir) {
    var keywords = Object.keys(h3s).sort(caseless_sort);
    var keyword_letters = {};
    // format output markdown based on outputdir
    var formatter = function(keyword) {
        if (outputdir) {
            return '<li class="functionli"><a href="' + h3s[keyword] + '#' + keyword + '">' + keyword + '</a></li>';
        } else {
            return '<li class="functionli"><a href="#' + keyword + '">' + keyword + '</a></li>';
        }
    };
    ...
    return '<code class="functionlist"><ul class="functionul">' + keywords_marked.join("\n") + '</ul></code>';
}
...
function indexer (h1s, outputdir) {
	...
	return '<span id="index">' + keywords_marked.join("\n") + '</span>';
}
...
// input: ([files], [h1s])
// output: [linked_files]
function autolink(files,h1s,output,h3s) {
	var keywords = Object.keys(h1s);
	var keywords3 = Object.keys(h3s);
	keywords.sort().reverse();
	keywords3.sort().reverse();
	keywords = keywords.map(function(kw){ return kw.replace(/\s/g,"\\s") });
	keywords3 = keywords3.map(function(kw){ 
	    kw = kw.replace('(','\134(');
	    kw = kw.replace(')','\134)');
	    kw = kw.replace('[','\134[');
        kw = kw.replace(']','\134]');
	...
	var keys3 = '(' + keywords3.join("|") + ')';
	var re = new RegExp(link_leadin.source + keys + link_outtro.source, 'g');
	var re3 = new RegExp(link_leadin3.source + keys3 + link_outtro3.source, 'g');
	var i,l,content;
	for (i = 0, l = files.length; i < l; i++) {
		...
		if (output) {
			...
			input = input.replace(re3,function(_,m3,m4) {
                return m3+'<a href="'+h3s[m4]+'#'+m4+'">'+m4+'<\/a>';
            });
            input = input.replace(/\<code\>\<a href="[^\#\>]*#/g,'<code><a name="');
		} else {
			input = input.replace(re, function(_,m1,m2) {
...
exports.h3finder = h3finder;
exports.indexer = indexer;
exports.h3index = h3index;
